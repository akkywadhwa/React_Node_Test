                            ...
                            <Card>
                                <Grid gap={4}>
                                    ...
                                    <GridItem colSpan={{ base: 2, md: 1 }}>
                                        ...
                                        {data?.related === 'Contact' && contactAccess?.view ? data?.allAttendees && data?.allAttendees.filter(item => item.type === 'contact').map((item) => {
                                            return (
                                                <Link to={`/contactView/${item.id}`}>
                                                    <Text color='brand.600' sx={{ '&:hover': { color: 'blue.500', textDecoration: 'underline' } }}>{item.name}</Text>
                                                </Link>
                                            )
                                        }) : data?.related === 'Lead' && leadAccess?.view ? data?.allAttendees && data?.allAttendees.filter(item => item.type === 'lead').map((item) => {
                                            return (
                                                <Link to={`/leadView/${item._id}`}>
                                                    <Text color='brand.600' sx={{ '&:hover': { color: 'blue.500', textDecoration: 'underline' } }}>{item.name}</Text>
                                                </Link>
                                            )
                                        }) : data?.related === 'contact' ? data?.allAttendes && data?.allAttendees.filter(item => item.type === 'contact').map((item) => {
                                            return (
                                                <Text color='blackAlpha.900' >{item.name}</Text>
                                            )
                                        }) : data?.related === 'lead' ? data?.allAttendes && data?.allAttendees.filter(item => item.type === 'lead').map((item) => {
                                            return (
                                                <Text color='blackAlpha.900' >{item.name}</Text>
                                            )
                                        }) : '-'}
